...
public class T4_compare {
	public static void main(String[] args) {
		...
		// 날짜와 시간 구분하기(T 문자를 기준으로 spilt()메소드 이용하여 자르기)
		...
		System.out.println("타켓(현재) 날짜/시간 : " + targetDateTime);
		System.out.println();
		
		String strTarget = targetDateTime.toString();
		//String bigyoToday = strtempToday.substring(0,strtempToday.indexOf(".")); / 필료없는 줄
		System.out.println("타켓(비교) 날짜/시간 : " + strTarget);
		
		// 날짜와 시간 구분하기(T 문자를 기준으로 spilt()메소드 이용하여 자르기)
		String bigyoDate = strTarget.split("T")[0];
		String bugyoTime = strTarget.split("T")[1];
		System.out.println("비교날짜 : " + bigyoDate);
		System.out.println("비교시간 : " + bugyoTime);
		System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		
		if(strDate.equals(bigyoDate)) System.out.println("같은날");
		else System.out.println("다른날");
		
		// 날짜를 날짜형식으로 비교하고자 한다면? isEqual() : 동일날짜비교, 이전날짜비교 : isBefore(), 이후날짜비교: isAfter(), 날짜차이:Period
...
//		System.out.println(lastDay + "달의 마지막 일자?" + YearMonth.from(bigyoDate2).atEndOfMonth());
		...
		System.out.println(lastDay + "달의 마지막 일자?" + YearMonth.from(LocalDate.parse(lastDay2, DateTimeFormatter.ofPattern("yyyy-MM-dd"))).atEndOfMonth());
